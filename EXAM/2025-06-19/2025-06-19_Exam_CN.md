...
> 
> 📝 **解题步骤：**
> 1. **t0**: A=0, B·C=1 → Set触发，Y保持或变化
> 2. **t1**: A=1 → Reset触发
> 3. **t2**: A=0, B·C=1 → Set
> 4. **t3**: A=1, B·C=1 → 冲突，看优先级
> 5. **t4**: A=1, B·C=0 → Reset持续
> 
> ⚠️ **注意：** 需要根据考卷上的具体电路图来分析！
...
> 📝 **做题方法：**
> 1. 从电路推导 X = f(A, B, C, X)
> 2. 令 X=0 求 f₀，令 X=1 求 f₁
> 3. 用卡诺图表示结果（必须！）
> 
> **假设电路表达式为 X = NOR(A, Y) 且 Y = NOR(B·C, X)：**
> 
> 将Y代入：X = NOT(A OR Y) = NOT(A OR NOT(B·C OR X))
> 
> **对X展开：**
> - 当X=0时：f₀(A,B,C) = NOT(A OR NOT(B·C OR 0)) = NOT(A OR NOT(B·C)) = NOT A · (B·C)
> - 当X=1时：f₁(A,B,C) = NOT(A OR NOT(B·C OR 1)) = NOT(A OR 0) = NOT A
> 
> **Shannon展开结果：**
> ```
...
> ```
> 
> 📌 **考试技巧：** 先从电路推导X=f(A,B,C,X)的完整表达式，再分别令X=0和X=1化简。
...
> 🧠 **AI参考解析 (AI Solution):**
> 
> **8位寄存器运算原理 (8-bit Arithmetic)：**
> - 8位无符号范围：0 ~ 255
> - 8位有符号范围：-128 ~ 127（补码表示）
> - 溢出处理：result mod 256
> 
> **通用解题步骤：**
> 1. 计算原始结果 R
> 2. 无符号值 = R mod 256
> 3. 有符号值：若 R mod 256 ≥ 128，则有符号值 = (R mod 256) - 256
> 
> **常见例题：**
> | 运算 | 结果 | mod 256 | unsigned | signed |
> |------|------|---------|----------|--------|
...
> ```
> Moore自动机: M = <X, S, Z, δ, ω, s₀>
>   δ: S × X → S      (状态转移函数)
>   ω: S → Z          (输出仅取决于当前状态)
> 
> Mealy自动机: M = <X, S, Z, δ, ω, s₀>
>   δ: S × X → S      (状态转移函数)  
>   ω: S × X → Z      (输出取决于当前状态和输入)
> ```
> 
> **记忆口诀：**
> - Moore = "More states" → 输出只看状态
> - Mealy = "More efficient" → 输出看状态+输入
> 
> 📌 **考试重点：** 必须精确写出δ和ω的定义域和值域！
...
> - 3个AND门（级联结构）
> 
> 📌 **考试技巧：** 记住模式 `sᵢ = xᵢ XOR (进位链)`，进位链是所有低位的AND
...
> ```
> 
> **VHDL操作符优先级 (Operator Precedence)：**
> | 优先级 | 操作符 |
> |--------|--------|
> | 最高 | not, abs |
> | | *, /, mod, rem |
...
> 
> **画图步骤 (Drawing Steps)：**
> 1. 找出所有时序元件（有`rising_edge`的信号）→ 画DFF
> 2. 识别算术运算 → 画加法器/比较器
> 3. 识别选择结构 → 画MUX
> 4. 连接所有元件，标注信号名
> 
...
> - 状态：T (Taken) 或 NT (Not Taken)
> - 规则：预测错误就翻转状态
> - 初始NT：第一次跳转必错
> 
> **2位预测器 (2-bit Saturating Counter)：**
> | 状态 | 预测 | T时 | NT时 |
> |------|------|-----|------|
...
> **外循环N次，内循环M次：**
> ```c
> for (i=0; i<N; i++)      // 外循环：N次T + 1次NT
>   for (j=0; j<M; j++)    // 内循环：每轮M次T + 1次NT，共N轮
> ```
> 
> **1位预测器 miss计算：**
> - 外循环：首次入循环miss + 最后退出miss = 2次
> - 内循环：每轮首次miss + 每轮退出miss = N×2次
> - **总计：2 + 2N次miss**
> 
> **2位预测器（初始WT）miss计算：**
> - 外循环：最后退出miss = 1次
> - 内循环：每轮退出从ST→WT不算miss，最后退出才miss
> - 规律：内循环每轮退出时1次miss
> - **总计：1 + N次miss**（约为1位的一半）
> 
> **典型例题：i<5, j<2000**
